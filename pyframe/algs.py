...
class ListBuilder(pyframe.core.Algorithm):
    ...
    def execute(self, weight):
        for prefix, key in zip(self.prefixes, self.keys):
            ...
            nparts = "" 
            if "jet_0" in prefix: nparts = "n_jets"      
            if "tau_0" in prefix: nparts = "n_taus"      
            if "lep_0" in prefix: nparts = "n_muons"     
    ...
    def execute(self, weight):
        ...
        for key in self.keys:
            ...
            for p in vec:
                ...
                elif p.prefix.startswith('jet_0_'):
                    p.tlv.SetPtEtaPhiM(p.pt, p.eta, p.phi, p.m)
                elif p.prefix.startswith('tau_0_'):
                    p.tlv.SetPtEtaPhiM(p.pt, p.eta, p.phi, p.m)
