...
class CutAlg(pyframe.core.Algorithm):
   ...
   #__________________________________________________________________________
    ...
    def cut_Filter(self):
        if self.sampletype == "mcTruth":
           return self.chain.tau_0_truth_isHadTau == 1
	elif self.sampletype == "mcAntiTruth":
	   return self.chain.tau_0_truth_isHadTau == 0
	else:
	   return True
    ...
    # MUON CUT
    #__________________________________________________________________________
    def cut_MuLowPt(self):
        #if "mc" in self.sampletype:
        #    run_number = self.chain.NOMINAL_pileup_random_run_number
        #else:
        #    run_number = self.chain.run_number
        #if run_number <= 276262 and run_number <= 284484:
        #    return self.chain.lep_0_pt < 40.0
        #elif run_number >= 296939 and run_number <= 300287:
        #    return self.chain.lep_0_pt < 40.0
        #elif run_number >= 300345 and run_number <= 302872:
        #    return self.chain.lep_0_pt < 50.0
        #elif run_number >= 302919:
        return self.chain.lep_0_pt < 50.0

    def cut_MuHighPt(self):
        #if "mc" in self.sampletype:
        #    run_number = self.chain.NOMINAL_pileup_random_run_number
        #else:
        #    run_number = self.chain.run_number
   ...
   #__________________________________________________________________________
	...
	return self.chain.tau_0_trig_HLT_tau25_medium1_tracktwo == 1

    def cut_HLTTau35Med1TrackTwo(self):
	return self.chain.tau_0_trig_HLT_tau35_medium1_tracktwo == 1

    def cut_HLTTau50L1TAU12Med1TrackTwo(self):
        return self.chain.tau_0_trig_HLT_tau50_medium1_tracktwo_L1TAU12 == 1

    def cut_HLTTau80Med1TrackTwo(self):
        return self.chain.tau_0_trig_HLT_tau80_medium1_tracktwo == 1

    def cut_HLTTau80L1TAU60Med1TrackTwo(self):
        return self.chain.tau_0_trig_HLT_tau80_medium1_tracktwo_L1TAU60 == 1

    def cut_HLTTau125Med1TrackTwo(self):
        return self.chain.tau_0_trig_HLT_tau125_medium1_tracktwo == 1

    def cut_HLTTau160Med1TrackTwo(self):
        return self.chain.tau_0_trig_HLT_tau160_medium1_tracktwo == 1

    def cut_HLTTauL1TAU12IMMed1TrackTwo(self):
        return self.chain.L1_TAU12IM_resurrected == 1

    def cut_HLTtau25perfptonly(self):
	return self.chain.tau_0_trig_HLT_tau25_idperf_tracktwo == 1

    def cut_HLTtau25perftracktwo(self):
	return self.chain.tau_0_trig_HLT_tau25_perf_tracktwo ==1
...
class PlotAlg(pyframe.algs.CutFlowAlg,CutAlg):
    ...
    def plot(self, region, passed, list_cuts, cut, list_weights=None, weight=1.0):
        ...
        self.h_HLT_tau_eta = self.hist('h_HLT_tau_eta', "ROOT.TH1F('$', ';  HLT tau #eta ;Events', 24, -6.0, 6.0)", dir=TAUS)

        self.h_HLT_tau_phi = self.hist('h_HLT_tau_phi', "ROOT.TH1F('$', ';  HLT tau #phi ;Events', 20, -5.0, 5.0)", dir=TAUS)

        self.h_HLT_tau_pt = self.hist('h_HLT_tau_pt', "ROOT.TH1F('$', ';HLT tau p_{T} [GeV];Events', 1000, 0.0, 1000.0)", dir=TAUS)

        self.h_HLT_jet_bdt_score = self.hist('h_HLT_jet_bdt_score', "ROOT.TH1F('$', ';  HLT tau BDT score ;Events', 54, -1.0, 2.0)", dir=TAUS)

        self.h_HLT_tau_n_tracks = self.hist('h_HLT_tau_n_tracks', "ROOT.TH1F('$', ';  HLT tau N_{tracks} ;Events', 5, 0.0, 5.0)", dir=TAUS)

        self.h_HLT_tau_n_wide_tracks = self.hist('h_HLT_tau_n_wide_tracks', "ROOT.TH1F('$', '; HLT tau N_{wide tracks} ;Events', 5, 0.0, 5.0)", dir=TAUS)

        self.h_HLT_pt_res = self.hist('h_HLT_pt_res', "ROOT.TH1F('$', ';HLT tau p_{T} resolution;Events', 2000, -10.0, 10.0)", dir=TAUS)
        ...
        self.h_HLT_fcent = self.hist('h_HLT_fcent', "ROOT.TH1F('$', ';HLT tau f_{cent};Events', 40, 0, 1.0)", dir=TAUS)

	self.h_HLT_ratio_energy_to_trk_p = self.hist('h_HLT_ratio_energy_to_trk_p', "ROOT.TH1F('$', ';HLT tau f_{track}^{EM};Events', 40, 0, 10)", dir=TAUS)

	self.h_HLT_frac_EM_from_charged_pions = self.hist('h_HLT_frac_EM_from_charged_pions', "ROOT.TH1F('$', ';HLT tau f_{EM}^{track-HAD};Events', 40, -1.0, 1.0)", dir=TAUS)

	self.h_HLT_lead_trk_p_frac = self.hist('h_HLT_lead_trk_p_frac', "ROOT.TH1F('$', ';HLT tau f^{-1}_{lead track};Events', 40, 0, 10)", dir=TAUS)

	self.h_HLT_trk_radius = self.hist('h_HLT_trk_radius', "ROOT.TH1F('$', ';HLT tau R_{track};Events', 40, 0, 0.2)", dir=TAUS)

	self.h_HLT_3P_dRmax = self.hist('h_HLT_3P_dRmax', "ROOT.TH1F('$', ';HLT tau #Delta R_{max};Events', 40, 0, 0.2)", dir=TAUS)

	self.h_HLT_1P_lead_trk_IP_sig = self.hist('h_HLT_1P_lead_trk_IP_sig', "ROOT.TH1F('$', ';HLT tau S_{lead track};Events', 40, 0, 2.0)", dir=TAUS)

	self.h_HLT_m_trk_EM_system = self.hist('h_HLT_m_trk_EM_system', "ROOT.TH1F('$', ';HLT tau m_{#pi ^{0} + track}[MeV];Events', 40, 0, 6000)", dir=TAUS)

	self.h_HLT_1P_frac_trks_iso_region = self.hist('h_HLT_1P_frac_trks_iso_region', "ROOT.TH1F('$', ';HLT tau f_{iso}^{track};Events', 40,  0, 1.0)", dir=TAUS)

	self.h_HLT_ratio_trkemsys_pt = self.hist('h_HLT_ratio_trkemsys_pt', "ROOT.TH1F('$', ';HLT tau P_{T}^{EM+track}/P_{T};Events', 40, 0, 3.0)", dir=TAUS)

	self.h_HLT_3P_massTrkSysCorrected = self.hist('h_HLT_3P_massTrkSysCorrected', "ROOT.TH1F('$', ';HLT tau m_{track}[MeV];Events', 40, 0, 6000)", dir=TAUS)

	self.h_HLT_3P_trans_flightpath_sig = self.hist('h_HLT_3P_trans_flightpath_sig', "ROOT.TH1F('$', ';HLT tau S_{T}^{flight};Events', 40, -10, 10)", dir=TAUS)
        ...
        self.h_tau_pt = self.hist('h_tau_pt', "ROOT.TH1F('$', ';Offline p_{T}(tau) [GeV];Events', 1000, 0.0, 1000.0)", dir=TAUS)
        ...
        self.h_tau_eta = self.hist('h_tau_eta', "ROOT.TH1F('$', ';Offline tau #eta ;Events', 12, -3.0, 3.0)", dir=TAUS)

        self.h_tau_phi = self.hist('h_tau_phi', "ROOT.TH1F('$', ';Offline #phi ;Events', 16, -4.0, 4.0)", dir=TAUS)

        self.h_mu_eta = self.hist('h_mu_eta', "ROOT.TH1F('$', '; muon #eta ;Events', 12, -3.0, 3.0)", dir=MUONS)

        self.h_mu_phi = self.hist('h_mu_phi', "ROOT.TH1F('$', '; muon #phi ;Events', 16, -4.0, 4.0)", dir=MUONS)

        self.h_met_reco_phi = self.hist('h_met_reco_phi', "ROOT.TH1F('$', ';MET #phi ;Events', 16, -4.0, 4.0)", dir=MET)

        self.h_tau_n_tracks = self.hist('h_tau_n_tracks', "ROOT.TH1F('$', ';Offline tau N_{tracks} ;Events', 5, 0.0, 5.0)", dir=TAUS)

	self.h_jet_bdt_score = self.hist('h_jet_bdt_score', "ROOT.TH1F('$', ';Offline tau BDT score ;Events', 20, 0.6, 1.0)", dir=TAUS)
